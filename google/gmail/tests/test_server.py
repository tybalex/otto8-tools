...
class TestEmailFunctions:
        ...
        with pytest.raises(ToolError, match="Please use the parameters `after` and `before`"):
            async with Client(mcp) as client:
                await client.call_tool(
                    name='list_emails',
                    arguments={'query': 'after:2024-01-01', 'cred_token': 'fake_token'}
                )
    ...
    @patch('server.get_client')
    async def test_read_email_missing_params(self, mock_get_client, mock_service):
        mock_get_client.return_value = mock_service
    ...
    @patch('server.get_client')
    async def test_delete_email_success(self, mock_get_client, mock_service):
        mock_get_client.return_value = mock_service
    ...
    @patch('server.get_client')
    async def test_get_current_email_address_success(self, mock_get_client, mock_service, mock_profile_data):
        mock_get_client.return_value = mock_service
...
class TestDraftFunctions:
    ...
    @patch('server.get_client')
    async def test_delete_draft_success(self, mock_get_client, mock_service):
        mock_get_client.return_value = mock_service
    ...
    @patch('server.get_client')
    async def test_send_draft_success(self, mock_get_client, mock_service):
        mock_get_client.return_value = mock_service
...
class TestLabelFunctions:
    ...
    async def test_create_label_success(self, mock_create_label_func, mock_get_client, mock_service):
        ...
        async with Client(mcp) as client:
            result = await client.call_tool(
                name='create_label',
                arguments={'label_name': 'New Label', 'cred_token': 'fake_token'}
            )
            res_json = json.loads(result[0].text)
...
class TestErrorHandling:
    ...
    @patch('server.get_client')
    async def test_send_email_http_error(self, mock_get_client, mock_service):
    ...
    @patch('server.get_client')
    async def test_delete_draft_http_error(self, mock_get_client, mock_service):
        ...
        with pytest.raises(ToolError):
            async with Client(mcp) as client:
                await client.call_tool(
                    name='delete_draft',
                    arguments={'draft_id': 'nonexistent_id', 'cred_token': 'fake_token'}
                )
