...
async def delete_tenant(tenant_id: str):
    async with AsyncSessionLocal() as session:
        await session.execute(
            delete(ChunkEntry).where(ChunkEntry.tenant_id == tenant_id)
        )
        await session.execute(
            delete(FileRecord).where(FileRecord.tenant_id == tenant_id)
        )
...
async def create_file(tenant_id: str, file_id: str, metadata: dict):
    async with AsyncSessionLocal() as session:
        stmt = (
            ...
            .values(tenant_id=tenant_id, file_id=file_id, file_metadata=metadata)
